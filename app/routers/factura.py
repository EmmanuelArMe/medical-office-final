...
def obtener_factura_por_id(factura_id: int, db: Session = Depends(get_db)):
    factura = service.obtener_factura_por_id(db, factura_id)
    return JSONResponse(
...
def eliminar_factura(factura_id: int, db: Session = Depends(get_db)):
    factura = service.eliminar_factura(db, factura_id)
    return JSONResponse(
        content={"message": f"Factura con ID {factura_id} eliminada correctamente", "response": jsonable_encoder(factura)},
        status_code=status.HTTP_200_OK
    )
...
def actualizar_factura(factura_id: int, factura_data: FacturaUpdate, db: Session = Depends(get_db)):
    factura_actualizada = service.actualizar_factura(db, factura_id, factura_data)
    return JSONResponse(
        content={"message": f"Factura con ID {factura_id} actualizada correctamente", "response": jsonable_encoder(factura_actualizada)},
        status_code=status.HTTP_200_OK
    )

@router.get(
        "/facturas/pago/{pago_id}",
        response_model=FacturaResponse,
        summary="Obtener factura por ID de pago",
        description="Obtiene una factura por su ID de pago."
)
def obtener_factura_por_pago_id(pago_id: int, db: Session = Depends(get_db)):
    factura = service.obtener_factura_por_pago_id(db, pago_id)
    return JSONResponse(
        content={"message": "Factura obtenida correctamente", "response": jsonable_encoder(factura)},
        status_code=status.HTTP_200_OK
    )